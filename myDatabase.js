...
myDatabase.prototype.postData = function(data,res) {
  let obj = {name:data.name, countDown:data.countDown, notes:data.notes};
...
console.log("getData");
  InfoModel.find({name:name},function(error,info) {
      ...
      if (info.length == 1)    
          return res.json({error:false, countDown:info[0].countDown, notes:info[0].notes});
      else
          return res.json({error:true});
   });
}
...
//  InfoModel.findOneAndUpdate({ident:data.ident},{name:data.name,gradeLevel:data.gradeLevel},function(error,oldData) {
  InfoModel.findOneAndUpdate({name:data.name},{countDown:data.countDown, notes:data.notes},function(error,oldData) {
    if (error) {
      return res.json({error:true});
    }
